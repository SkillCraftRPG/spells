<template>
  <main class="container-fluid page">
    <h1>{{ title }}</h1>
    <p class="text-body-secondary">{{ spells.length }} School of Illusion spells</p>
...
const spells = computed<Spell[]>(
  () =>
    orderBy(
      spellData.filter((spell) => spell.school === "Illusion").map((spell) => ({ ...spell, sort: [spell.level, spell.name].join("_") })),
      "sort",
    ) as Spell[],
);
const title = computed<string>(() => t("spells.title.illusion"));
