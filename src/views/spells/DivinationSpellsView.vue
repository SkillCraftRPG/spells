<template>
  <main class="container-fluid page">
    <h1>{{ title }}</h1>
    <p class="text-body-secondary">{{ spells.length }} School of Divination spells</p>
...
const spells = computed<Spell[]>(
  () =>
    orderBy(
      spellData.filter((spell) => spell.school === "Divination").map((spell) => ({ ...spell, sort: [spell.level, spell.name].join("_") })),
      "sort",
    ) as Spell[],
);
