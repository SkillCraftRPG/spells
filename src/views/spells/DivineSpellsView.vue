<template>
  <main class="container-fluid page">
    <h1>{{ title }}</h1>
    <p class="text-body-secondary">{{ spells.length }} Cleric & Paladin spells</p>
...
const spells = computed<Spell[]>(
  () =>
    orderBy(
      spellData
        .filter((spell) => spell.classes.includes("Cleric") || spell.classes.includes("Paladin"))
        .map((spell) => ({ ...spell, sort: [spell.level, spell.name].join("_") })),
      "sort",
    ) as Spell[],
);
const title = computed<string>(() => t("spells.title.divine"));
